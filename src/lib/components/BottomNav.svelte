<script lang="ts">
  ...
  let activeRoute = $derived.by(() => normalizeRoute(stripBase(page.url.pathname)));

  let activeIndex = $derived.by(() => {
    const idx = navItems.findIndex(item => item.path === activeRoute);
    return idx >= 0 ? idx : 0;
  });
</script>
    ...
    {#each navItems as item, i}
      <li class="flex-1">
        <button
          ...
          class="w-full flex flex-col items-center justify-center h-full space-y-1 transition-all duration-300 group"
